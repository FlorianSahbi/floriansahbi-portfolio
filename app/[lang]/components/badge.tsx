...
export default function Badge({
  ...
  const variants = {
    default: clsx(
      ...
      hoverable &&
        'transition-colors duration-200 hover:bg-zinc-900/20 hover:text-zinc-300',
    ),
    condensed: clsx(
      ...
      hoverable &&
        'transition-colors duration-200 hover:bg-zinc-900/10 hover:text-zinc-300',
    ),
    tag: clsx(
      'border rounded-md py-[2px] px-2 text-xs font-medium',
      'bg-zinc-800/20 border-zinc-700 text-zinc-400',
      hoverable &&
        'transition-colors duration-200 hover:bg-zinc-700/10 hover:text-zinc-300',
    ),
  }
  ...
  const content = (
    <div className="flex items-center gap-1">
      {icon && (
        <Icon size={variant === 'condensed' || variant === 'tag' ? 12 : 16} />
      )}
      <span>{label}</span>
    </div>
  )
