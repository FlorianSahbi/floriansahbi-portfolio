...
export default function ProjectCard({
  ...
  return (
    <Card active={active} key={slug}>
      <Link href={href}>
        <article className={clsx("p-4 md:p-8", isFeatured && "relative w-full h-full")}>
          <h2
            className={clsx(
              "z-20 font-display duration-1000 group-hover:text-white",
              isFeatured
                ? "mt-4 text-3xl font-bold text-zinc-100 sm:text-4xl"
                : "text-xl font-medium lg:text-3xl text-zinc-200"
            )}
          >
            {title}
          </h2>
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-2 overflow-hidden mt-4">
              {tags.map((tag, idx) => (
          ...
          <div
            className={clsx(
              ...
              isFeatured
                ? "absolute bottom-4 md:bottom-8 hidden lg:block"
                : "z-20 mt-4"
            )}
          >
